...
joblib.dump(preprocessor, "new_preprocessor.joblib")
...
x_new = pd.DataFrame(dict(
	airline=[airline],
	date_of_journey=[doj],  # pd.to_datetime(doj)
	source=[source],
...
#st.write(x_new.dtypes)
#st.write(x_new)


if st.button("Predict"):
    saved_preprocessor = joblib.load("new_preprocessor.joblib")
    x_new_pre = saved_preprocessor.transform(x_new)
    #st.write(x_new_pre)
    #st.write(x_new_pre.dtypes)

    with open("xgb_model.pkl", "rb") as f:
        best_model = pickle.load(f)


    pred = np.round(best_model.predict(x_new_pre),2)
